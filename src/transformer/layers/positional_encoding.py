...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, sequence_length, dropout):
        ...
        div_term = torch.exp(torch.arange(0, d_model, 2).float().to(self.DEVICE) * (-torch.log(torch.tensor([self.PE_CONSTANT], device=self.DEVICE)) / d_model))

        self.pos_encoding = torch.zeros(sequence_length, d_model, device=self.DEVICE)
        self.pos_encoding[:, 0::2] = torch.sin(position * div_term).to(self.DEVICE)
        self.pos_encoding[:, 1::2] = torch.cos(position * div_term).to(self.DEVICE)
